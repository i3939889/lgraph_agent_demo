...
一個基於終端機介面 (CLI) 的 RAG (檢索增強生成) 聊天機器人，使用 Google Gemini API 與 LlamaIndex 進行高效的知識庫查詢。目前版本為 **第一階段 (Stateless MVP)**，專注於提供嚴格且具防幻覺機制的知識對答。
...
* **Google Gemini 2.0 支援**：使用目前最新的 `gemini-2.0-flash` 模型與 `gemini-embedding-001` 進行強大的語言生成與語意理解。
